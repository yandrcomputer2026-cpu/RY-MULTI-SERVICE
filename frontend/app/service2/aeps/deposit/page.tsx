...
export default function AepsDepositPage() {
  ...
  if (!Number.isFinite(cleanAmount) || cleanAmount <= 0) {
    setMessage("कृपया सही Deposit Amount डालें।");
    return;
  }

  if (!consent) {
    setMessage(
      "AEPS Cash Deposit के लिए consent देना जरूरी है।",
    );
    return;
  }
  ...
  try {
    ...
    if (!providerReady) {
      setMessage(
        "AEPS provider अभी active नहीं है। इसलिए Cash Deposit process शुरू नहीं किया गया है।",
      );
      return;
    }

    setMessage(
      "AEPS provider active है, लेकिन supported live Cash Deposit workflow अभी implement और verify नहीं किया गया है। कोई transaction process नहीं किया गया।",
    );
  } catch (error) {
    console.error(
      "AEPS CASH DEPOSIT STATUS ERROR:",
      error,
    );
        ...
        <div className="rounded-2xl border bg-white p-8 shadow-sm">
          ...
          <div className="mt-6 rounded-xl border border-amber-200 bg-amber-50 p-4">
            ...
            <p className="mt-2 text-sm leading-6 text-amber-800">
              Authorized AEPS provider, supported cash deposit flow और
              biometric authentication connect होने के बाद ही real Cash
              Deposit enable होगा।
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="mt-8">
            ...
            <label className="mt-6 flex cursor-pointer items-start gap-3 rounded-xl bg-gray-50 p-4">
              ...
              <span className="text-sm leading-6 text-gray-600">
                मैं समझता/समझती हूँ कि AEPS Cash Deposit केवल authorized
