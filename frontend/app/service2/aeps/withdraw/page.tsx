...
async function handleSubmit(event: FormEvent<HTMLFormElement>) {
  ...
  try {
    ...
    if (!providerReady) {
      setMessage(
        "AEPS provider अभी active नहीं है। इसलिए Cash Withdrawal process शुरू नहीं किया गया है।",
      );
      return;
    }

    setMessage(
      "AEPS provider active है, लेकिन live Cash Withdrawal workflow अभी implement नहीं किया गया है। कोई transaction process नहीं किया गया।",
    );
  } catch (error) {
    console.error(
      "AEPS CASH WITHDRAWAL STATUS ERROR:",
      error,
    );
